...
public class CustomRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		Admin admin = null;
		try {
			admin = sysService.findSysUserByUserCode(userCode);
		} catch (Exception e1) {
		...
		if(!admin.getAvailable().trim().equals("1")) try {
//			throw new Exception("用户被禁止使用，请联系管理员解封");
			return null;
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		activeUser.setUsercode(admin.getAccount());
		...
		try {
			//通过service取出菜单
			menus = sysService.findMenuListByUserId(admin.getA_id());
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
